...
trainstat_graph <- ggplot(train_data2, aes(x = train_data2$status)) +
  geom_bar() +
   theme(axis.text.x = element_text(angle=90, hjust=1)) + coord_cartesian(xlim = c(0, 10))
...
trainorig_graph <- ggplot(train_data2, aes(x = train_data2$origin)) +
  geom_bar() +
   theme(axis.text.x = element_text(angle=90, hjust=1)) + coord_cartesian(xlim = c(0, 5))
...
train_data2 <- within(train_data2,
                      train_data2$wkday <- factor(train_data2$wkday,
                                         levels = names(sort(table(factor(train_data2$wkday)),
                                                              decreasing = TRUE))))
...
trainid_graph <- ggplot(train_data2, aes(x = train_data2$train_id)) +
  geom_bar() +
   theme(axis.text.x = element_text(angle=90, hjust=1))+ coord_cartesian(xlim = c(0, 50))
...
#DataTree with regression module
```{r}
#create a new dataframe with only relevant variables that impact the data.
#head(train_data)
train_data_regr <- train_data2[,c("status", "origin", "hour", "minute", "month", "next_station")]
#head(train_data_regr)


#train_data_regr$status <- as.numeric(train_data_new$status)

head(train_data_regr$status)
...
#create split
#create split
split = sample.split(train_data_regr$status, SplitRatio = 0.8)

#create training and test set using the subset function

Trainregset = subset(train_data_regr, split == TRUE)
Testregset = subset(train_data_regr, split == FALSE)
...
#now create the CART model. Use rpart to build a classification tree
#use rpart formula to fit the data.
TraindataregrTree = rpart(status ~ origin+ hour + minute + month + next_station, method = "anova", data = Trainregset)


printcp(TraindataregrTree)
plotcp(TraindataregrTree)
summary(TraindataregrTree)
rsq.rpart(TraindataregrTree)
##plot the tree

prp(TraindataregrTree)
