...
trainnext_graph <- ggplot(train_data, aes(x = train_var)) +
  geom_bar() +
...
####Testing max depth on test set. The depths start leveling around the sixth level. There is no need to increase the depth more than the sixth level.
```{r}
#R2ts <- numeric()
R2ts <- 0
for (iv in 1:10){
